...
> ✅ 已完成（2026-06-14 晚）：**盖盖子 + 3修复 + 导出误删bug** 全部上线，三边一致。① 金句星标/AI推荐/片头音乐/后台AI批改 用开关藏起(`SHOW_GOLDEN`/`SHOW_AI_REVIEW`=false，想恢复改 true)，「重新提交审核」保留；② #7/#12/咔哒声 推正式站；③ 修复「导出误删保留内容」P0(合并阈值 `1.5→0.08` `MERGE_GAP`)。详见下方「最近完成」和开发日记。
>
> 🔴 **待办·必须做**：通知 **26 位受影响学员**重新打开项目→重新导出→重新提交（剪辑数据未丢、不用重剪）。名单+手机尾号在 2026-06-14 日记/对话记录。合计误删 30.9 分钟保留内容。

| 优先级 | 事项 | 状态 |
|---|---|---|
| A | 原音频迁 OSS+CDN 下载链路 | 学员反馈 Windows 下载原音频卡住 20-30 分钟；确认原音频下载是高频需求，当前仍由 ECS 本地 `/uploads/` 直发，带宽慢。已写完成定义 `2026-06-16_原音频迁OSS-CDN完成定义.md`，待当当确认后开工 |
| A | 🔴 关闭验证码兜底（安全洞） | 正式站 `ALLOW_DEV_SEND_CODE_FALLBACK=1` 时验证码直接在接口返回，且管理员身份只认手机号；任何人可用管理员手机号登成管理员。需补齐阿里云短信签名/模板后关闭兜底；补不齐前可先给后台加二道口令 |
| A | 审核口径待用户拍板 | 直播口径是"审核通过后进接单台"，系统现状是"提交一次审核快照即解锁，打回不重锁"。要么改系统要么统一对学员口径，2026-06-13 仍未定 |
| A | 剪辑决策恢复旧版脑子 | 未完成，有执行计划，见追溯台账；学员 D2/D3 正在剪开营直播，优先级高 |
| A | 多音频上传线上复测 | 本地完成，线上待部署/复测 |
| A | 上传重复验证码排查 | 需真机复现 |
| A | 提交快照给助教审核最小闭环 | 部分完成，仍需收口 |
| A | 我的项目为空排查 | 需线上复现 |
| A | 派单入口和权限 | 本地基本完成，线上待验收 |
| B | 上传存储从 ECS 本地盘迁到 OSS | 2026-06-13 只读确认：当前 `/api/upload` 真实写入 ECS `/opt/money-scissors-m2/uploads`，返回 `bucket=ecs-local`；正式约 2.2G，测试约 435M；线上 `.env` 已无阿里云 AK，OSS 路由未启用。清理脚本默认清 `public/uploads`，与真实大头目录不一致，且 cron 未配置。短期能跑，长期需迁 OSS 或修正本地清理/备份策略 |
| B | 21天占位功能按课表倒排 | D8(6/19)/D10/D15 模板领取、D20(7/1) 账本提问还是灰色占位，D8 最早到期 |
| B | 测试站独立门牌 | 2026-06-13 已分家并可公网访问：`http://8.136.133.196:8090/` 独立 server 块整体转发到测试后端 3004，测试站独立 JWT_SECRET，正式站(80→3002)零改动。备份 `/root/nginx-backups/test-split-8090-20260613-193323/`。8090=不同 origin，localStorage 与正式站天然隔离，验收=8090 与 :80 登录互不影响 |
| B | Day1 卡片四问和金黄配色 | 本地完成，待用户看样式 |
| C | edit.html 独立锁页旧文案 | edit.html 有自己一套 Day1 锁页，没吃到"不是网站坏了"新文案 |
| C | 部署脚本 smoke 误报 | `deploy_ecs.sh` 末尾仍按旧免登录逻辑期待 `/api/auth/me=200`，登录开启后正确结果是 401，每次误报 |
