...
> 这条是窗口收口规则，不要只当成长期记忆。每次发最后一条消息前必须自检。

- 如果任务已经真正完成，并且有完成证据/验收结果，可以正常收口。
- 如果任务处于等待、冻结、卡住、待当当体验、待总调度裁决、待外部结果、待权限/密钥/真人确认，全部视为“未完成”。
- 未完成时，最后一条消息不能只用句号结束，必须列：
  1. 当前进度
  2. 阻塞点/未完成项
  3. 2-3 个带问号的下一步选择
  4. 钱钱/执行窗口推荐哪一个
- 只复述“我记住了”不算执行；最后一条消息必须真的按这个格式写出来。
