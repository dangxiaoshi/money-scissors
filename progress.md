...
1. **review.html『本单需求』入口**（feature: review-dispatch-requirement）— 8090 已自验，等真账号真音频看一眼
2. **orders 接单页前端打磨**（feature: orders-frontend-polish）— 8090 已自验，等真账号看一眼

验收办法：登录 8090 对应页面走一遍 → 绿了走白名单单文件升正式（备份+对账+health 200）。
...
- **有人看着的代码窗口**：从 `feedback-entry` / `external-submission-backend` 里挑一个（都碰 server.cjs，需有人盯）。
- **当当**：先把上面两条验收债验收升正式；OSS 观察如果白天链路正常且带宽无异常，可把 `oss-prod-watch` 收成 done。
...
- [x] 改了代码的：node --check / 语法检查跑过，必要时 check_sync 对账（本轮未改代码；`./init.sh` 已确认 `server.cjs` 语法 OK）
- [x] feature_list.json 对应任务状态已更新，且 passing/done 都有 evidence（`oss-prod-watch` 仍 in_progress，已追加 evidence）
- [x] 本页『上一轮做了什么』『验收债』『下一轮从哪继续』已更新
- [x] 临时文件已清理（未创建临时文件）
- [x] 没有越围栏（夜间没碰 night_safe=false 的任务、没碰正式站配置/.env/数据库/SSH；只做公网 GET 健康探测）
